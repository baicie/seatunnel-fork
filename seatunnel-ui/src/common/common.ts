...
import { ITaskState, ITaskStateConfig, ITaskType } from './types'
import { h } from 'vue'
 ...
 * State code table
 */
export const stateType = (t: any, type: ITaskType = 'workflow') => [
  {
    value: '',
  ...
  ...Object.entries(tasksState(t))
    .filter(([, item]) => {
      const includeCondition = item.include ? item.include.includes(type) : true
      const excludeCondition =
        item.exclude && type ? !item.exclude.includes(type) : true
...
export const tasksState = (t: any): ITaskStateConfig => ({
  ...
  READY_KILL: {
    id: 23,
    desc: `${t('project.overview.ready_kill')}`,
    color: '#5102ce',
    icon: ThunderboltOutlined,
    isSpin: false,
    classNames: 'ready_kill',
    exclude: ['instance']
  },
  READY_RECOVER: {
    id: 24,
    desc: `${t('project.overview.ready_recovery')}`,
    color: '#5102ce',
    icon: RetweetOutlined,
    isSpin: false,
    classNames: 'ready_recovery',
    exclude: ['instance']
  }
})
...
export function renderTaskStateIcon(t: any, state: ITaskState) {
...
export const formatNumber = (num: number, fractionDigits = 6) => {
  return parseFloat(num.toFixed(fractionDigits))
}

export const ServiceStatus = {
  RUNNING: 'running',
  WAITING: 'maintain_able',
  STOPPED: 'maintain_able',
  OUT_OF_SERVICE: 'out_of_service',
  MAINTAIN_ABLE: 'maintain_able',
  LICENSE_CHECK_FAIL: 'license_check_fail',
  SERVICE_IS_BUSY: 'service_is_busy'
} as any
