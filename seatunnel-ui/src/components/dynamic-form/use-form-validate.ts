...
export function useFormValidate(forms: Array<any>, model: any, t: any) {
  ...
  const setValidate = (validate: any, field: string, label: string): object => {
    const data: any = {
        ...
        if (!model[field]) {
          return Error(
            validate.message === 'required'
              ? `${t(label)}${t('virtualTables.required')}`
              : t(validate.message)
          )
        }
      }
  ...
  forms.forEach((f: any) => {
    ...
    validate[f.field] = setValidate(f.validate, f.field, f.label)
  })
