...
import type { ISeriesData, ITaskRuntimeContext, ITaskState } from './types'
  ...
  setup(props) {
    ...
    props.seriesData.forEach(function (task, index) {
      ...
      data[task.status].push({
        name: task.name,
        taskRuntimeContext: task.taskRuntimeContext,
        value: [index, start, end, end - start]
      })
    })

    const getMarkPointValue = (taskRuntimeContext: ITaskRuntimeContext) => {
      if (!taskRuntimeContext) {
        return ''
      }

      if (taskRuntimeContext.hasBeenCoronatted) {
        return t('project.workflow.coronation')
      }
      if (taskRuntimeContext.hasBeenIsolated) {
        return t('project.workflow.isolation')
      }
      return ''
    }
    ...
    const series = Object.keys(state).map((key) => ({
      ...
      markPoint: {
        data: data[key].map((item) => ({
          name: item.name,
          value: getMarkPointValue(item.taskRuntimeContext),
          coord: [item.value[1], item.name],
          label: {
            formatter: function (params: any) {
              return params.value
            },
            color: locale.value === 'zh_CN' ? '#fff' : '#000'
          },
          itemStyle: {
            opacity:
              item?.taskRuntimeContext?.hasBeenCoronatted ||
              item?.taskRuntimeContext?.hasBeenIsolated
                ? 1
                : 0
          }
        }))
