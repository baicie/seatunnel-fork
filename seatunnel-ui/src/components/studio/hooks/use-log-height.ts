...
export function useLogHeight() {
  const { currentTabRef } = inject(configProviderInjectionKey, {
    currentTabRef: ref(-1)
  })
  ...
  const setLogHeight = (height: number) => {
    layoutStore.setFileLogHeight(currentTabRef.value, height)
  }
  ...
  const setCurrentLogHeight = () => {
    ...
    layoutStore.setLogHeightByFileId(currentTabRef.value)
  }
  ...
  return {
    ...
    setLogHeightByFileId: layoutStore.setLogHeightByFileId
  }
}
